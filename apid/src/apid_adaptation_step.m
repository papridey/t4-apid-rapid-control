function [EF_cmd_next, A_next, state] = apid_adaptation_step( ...
    y_state, yk, T4_des, prev_y_for_cost, dmeas_f_prev_for_cost, ...
    timing, ctrl, P, state, u_prev, t_now)
%APID_ADAPTATION_STEP One APID gain-update and control-update step.
    ...
    grad_Kp = fd_grad(@(x) window_cost_ISE_apid( ...
        y_state, x, state.Ki, state.Kd, T4_des, yk, prev_y_for_cost, ...
        dmeas_f_prev_for_cost, state.int_e, timing.Ts, u_prev, state.A_hold, ...
        state.A_basal, state.first_band_hit, ctrl.EF_min, ctrl.EF_max, ...
        ctrl.EF_rate, ctrl.Kb_aw, ctrl.thresh, ctrl.kA, ctrl.A_min, ...
        ctrl.A_max, ctrl.dA_max, ctrl.A_filter_fac, timing.T_total, ...
        timing.T_cycle, timing.T_on, t_now, timing.pulseDuty, timing.t5, ...
        timing.t6, ctrl.rhoA, ctrl.beta_ov, P, ctrl.alpha_d), state.Kp, epsKp);

    grad_Ki = fd_grad(@(x) window_cost_ISE_apid( ...
        y_state, state.Kp, x, state.Kd, T4_des, yk, prev_y_for_cost, ...
        dmeas_f_prev_for_cost, state.int_e, timing.Ts, u_prev, state.A_hold, ...
        state.A_basal, state.first_band_hit, ctrl.EF_min, ctrl.EF_max, ...
        ctrl.EF_rate, ctrl.Kb_aw, ctrl.thresh, ctrl.kA, ctrl.A_min, ...
        ctrl.A_max, ctrl.dA_max, ctrl.A_filter_fac, timing.T_total, ...
        timing.T_cycle, timing.T_on, t_now, timing.pulseDuty, timing.t5, ...
        timing.t6, ctrl.rhoA, ctrl.beta_ov, P, ctrl.alpha_d), state.Ki, epsKi);

    grad_Kd = fd_grad(@(x) window_cost_ISE_apid( ...
        y_state, state.Kp, state.Ki, x, T4_des, yk, prev_y_for_cost, ...
        dmeas_f_prev_for_cost, state.int_e, timing.Ts, u_prev, state.A_hold, ...
        state.A_basal, state.first_band_hit, ctrl.EF_min, ctrl.EF_max, ...
        ctrl.EF_rate, ctrl.Kb_aw, ctrl.thresh, ctrl.kA, ctrl.A_min, ...
        ctrl.A_max, ctrl.dA_max, ctrl.A_filter_fac, timing.T_total, ...
        timing.T_cycle, timing.T_on, t_now, timing.pulseDuty, timing.t5, ...
        timing.t6, ctrl.rhoA, ctrl.beta_ov, P, ctrl.alpha_d), state.Kd, epsKd);
    ...
    if state.first_band_hit
        A_next = max(A_next, 0.98 * state.A_basal);
    end
end
