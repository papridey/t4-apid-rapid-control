function OUT = run_apid_nobandlock_smooth(T4_des, opts)
%RUN_APID_NOBANDLOCK_SMOOTH
% Windowed adaptive PID without band-lock holding for EF-responsive T4 control.
%
% This version is intentionally kept as close as possible to
% run_apid_bandlock_smooth.m, with only the supervisory band-lock logic removed.
%
% Example:u
%   OUT = run_apid_nobandlock_smooth(25);
%   OUT = run_apid_nobandlock_smooth(25, struct('dt_seconds', 0.1/12));
    ...
    % Disable band-lock behavior completely
    ...
    for k = 1:timing.Nsteps-1
        ...
        if m <= numel(timing.sample_times) && timing.time(k+1) >= timing.sample_times(m)
            ...
            cost_baseline = window_cost_ISE_apid( ...
                ...
                u_pid(max(k,1)), state.A_hold, state.A_basal, false, ...
            ...
            % Keep band-lock disabled after helper call too
            state.band_locked    = false;
            state.first_band_hit = false;
            ...
            if opts.verbose
                fprintf(['win %2d t=%6.0f min | y=%.2f e=%.2f | ', ...
                         'Kp=%.3g Ki=%.3g Kd=%.3g | J=%.3g | A=%.6f\n'], ...
                         m-1, timing.time(k+1), yk, e_meas, ...
                         state.Kp, state.Ki, state.Kd, cost_baseline, state.A_hold);
            end
