...
fprintf('\n=== Running RAPID sweep over multiple setpoints ===\n');
...
%legend('Location','best');
% ============================================================
% Overlay plot: normalized robust costs
% ============================================================
% ============================================================
% Overlay plot: smoothed robust costs
% ============================================================
subplot(2,2,4); hold on;
for j = 1:Nsp
    OUT = OUTS{j};
    maskJ = ~isnan(OUT.cost_hist);
    tJ = OUT.sample_times(maskJ)/60;
    J  = OUT.cost_hist(maskJ);

    if isempty(J)
        continue;
    end

    Js = movmean(J, 3);   % or 5 if you want smoother curves

    plot(tJ, Js, 'LineWidth', 1.5, ...
        'DisplayName', sprintf('SP=%g', setpoints(j)));
end
xlabel('Time (h)');
ylabel('J_{rob}');
title('Smoothed per-window robust cost');
grid on;
legend('Location','best');
